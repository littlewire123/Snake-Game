...
- 请求码 （四字节，0表示改变方向，1创建或搜索房间，2获取已有房间，3注册，4登录，5上传存档，6下载经典模式存档，7下载挑战模式存档，8下载道具模式存档，9退出房间......）
...
- 有效数据长度（开头四字节）
- 数据类型（往后四字节，0：地图数据`struct map_t`，1：玩家蛇的位置数据`struct snake_data`，2：食物位置信息`struct food_t`，3：玩家控制信息`struct direction_t`，4：用户id信息`int32_t`，5：房间信息`room_t`，6：状态信息`status_t`，表示处理成功或者失败，7：用户账号密码信息`user_info_t`，8：文件`file_t`）
- 数据正文，长度为开头的四字节
- 包尾（包分隔符）：`\0`
...
- 解决方案：自己基于传输层协议（tcp，udp）封装一个应用层通信协议，用于快速编码解码，避免延迟过高，同时引入了请求码的概念，用于区分客户端的请求类型。同时为了确保数据可靠性采用了基于tcp协议传输，具体协议见第二大点。
...
- 更新：因为引入了创建房间功能，每一个房间都是独立的，因此每个房间都需要单独处理蛇的移动和发送蛇状态给客户端，所以把接收客户端请求的工作移动到了主线程，之后每创建一个房间就为其创建一个子线程来处理相关游戏工作。
...
- 问题：当一个房间的所有玩家都退出时，服务器程序会崩溃
- 问题原因：在原来的设计中，处理游戏房间的子线程中，游戏逻辑类是通过一个map的迭代器获取和操作的，而在主线程中如果所有玩家都退出了这个房间，房间就会被销毁，这个迭代器就会失效，就会导致程序异常终止。
- 解决方案：在每一次进行蛇的移动时都重新进行查找房间，如果房间已经被从map中删除，就会查找失败，这时就可以退出死循环并销毁线程（后续也许会引入线程池）。由于每次移动都要查找，这时红黑树超高的查找效率（logn）就起到很大的帮助了。同时由于其他线程也会对map进行操作，所以要对map加互斥锁，且要覆盖移动操作的全程，确保其原子性，防止进行到一半时元素被删除，导致操作的迭代器失效而崩溃。
