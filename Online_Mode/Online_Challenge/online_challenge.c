...
void print_online_challenge_map()
{
    ...
    for (i = 0; i < map->height; ++i)
    {
        for (j = 0; j < map->width; ++j)
        {
            ...
            else if ((user_id = online_find_challenge_body(find_body)) != -1)
            {
                if (user_id == id)
                {
                    GREEN_TEXT();
            ...
            else if (online_find_challenge_obstacle(find_obstacle) != NULL)
            {
                MAGENTA_TEXT();
                printf("☢");
                RESET_TEXT();
            }
            else if (online_find_challenge_food(find_food) != NULL)
            {
                RED_TEXT();
                printf("❤");
                RESET_TEXT();
